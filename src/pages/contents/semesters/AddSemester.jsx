...
function AddSemester({ refetch }) {
  ...
  const onSubmit = async (data) => {
    try {
      setLoading(true);
      const response = await API.post(`/content/semester/create`, {
      ...
      message.error(error.response?.data?.message || "Failed to add Semester");
    } finally {
      setLoading(false);
    }
  };
            ...
            <Controller
              ...
              render={({ field }) => (
                <InputNumber
                  {...field}
                  className="w-full"
                  placeholder="Enter Semester price"
                  min={0}
